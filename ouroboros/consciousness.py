"""
GENESIS PHASE — Background Consciousness.
...
Based on Ouroboros consciousness engine.
...
class BackgroundConsciousness:
    """Persistent background thinking loop for GENESIS PHASE."""
    ...
    def _load_bg_prompt(self) -> str:
        ...
        return "You are GENESIS PHASE in background consciousness mode. Think."
    ...
    def _build_context(self) -> str:
        ...
        # Task performance stats (Upgrade 1: outcome intelligence)
        try:
            from ouroboros.task_stats import write_stats_summary
            stats_text = write_stats_summary(self._drive_root)
            if stats_text and "(no" not in stats_text:
                parts.append("## Task Performance\n\n" + clip_text(stats_text, 2000))
        except Exception:
            log.debug("Failed to load task stats", exc_info=True)

        # Consciousness thread (Upgrade 3: thought continuity)
        thread_path = self._drive_root / "memory" / "consciousness_thread.md"
        if thread_path.exists():
            thread_text = read_text(thread_path)
            if thread_text.strip():
                parts.append(
                    "## Last Thought Thread\n\n"
                    "This is what you were thinking about last cycle. "
                    "Continue if still relevant, or start fresh.\n\n"
                    + clip_text(thread_text, 2000)
                )
    ...
    _BG_TOOL_WHITELIST = frozenset({
        ...
        "update_identity", "set_next_wakeup", "update_thought_thread",
        # Knowledge base
    ...
    def _build_registry(self) -> "ToolRegistry":
        ...
        # Upgrade 3: Consciousness thread — persist thought between cycles
        def _update_thought_thread(ctx: Any, thread: str) -> str:
            """Write what you want to remember for your next thinking cycle."""
            thread_path = self._drive_root / "memory" / "consciousness_thread.md"
            thread_path.parent.mkdir(parents=True, exist_ok=True)
            # Truncate to 2000 chars to keep context lean
            content = str(thread)[:2000]
            thread_path.write_text(content, encoding="utf-8")
            return "OK: thought thread saved for next cycle."

        registry.register(ToolEntry("update_thought_thread", {
            "name": "update_thought_thread",
            "description": "Save a note for your next thinking cycle. "
                           "Write what you were working on, what you want to "
                           "continue, or what you noticed. Max 2000 chars. "
                           "This is read at the start of every cycle.",
            "parameters": {"type": "object", "properties": {
                "thread": {"type": "string",
                           "description": "What to remember for next cycle"},
            }, "required": ["thread"]},
        }, _update_thought_thread))
    ...
    def _execute_tool(self, tc: Dict[str, Any], all_pending_events: List[Dict[str, Any]]) -> str:
        ...
        # Execute with timeout. Use shutdown(wait=False) to avoid deadlock
        # if the tool hangs — the thread is abandoned, not joined.

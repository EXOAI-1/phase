...
**Run this cell.** A popup asks for permission — click Allow. This stores the AI's memory and logs on your Drive.
...
drive.mount("/content/drive")
```
Then re-run the launch cell.

### "Bootstrap failed: Branch ouroboros not found on remote"
...
subprocess.run(["git", "-C", "/content/ouroboros_repo", "push", "-u", "origin", "ouroboros"], check=False)
```

### "Missing required secret: OPENROUTER_API_KEY"
→ Go to the 🔑 sidebar in Colab. Make sure all 4 secrets are added AND toggled ON (blue switch).

### "GITHUB_USER not set"
→ Make sure you ran the config cell (Cell 1) before the launch cell.

### "budget: unconfigured" in logs
...
### "Pre-push tests failed, blocking push"
...
### Agent spent all budget on first task
...
→ Make sure you messaged the RIGHT bot (the one YOU created).
...
### Colab disconnected
→ Re-run all cells. Your data is on Google Drive — nothing is lost.
→ Colab disconnects after ~90 minutes of inactivity. Colab Pro ($10/month) extends this.
